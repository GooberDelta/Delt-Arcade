...
3. Setup the Database "delt_arcade" with the collections "cards" "news" "transactions" "users".
...
5. Prepare to run the WebUI. Open the WebUI Folder and run this via python or by pip `pip install fastapi uvicorn pymongo python-jose[cryptography] passlib[bcrypt] python-dotenv bcrypt==4.0.1`. These are the dependecies for the webserver. (This will be cleaned up into a requirements later.)
6. set the admin via `py -c "from webui import create_admin;create_admin()"`. This will generate you an adminstrator account.
7. run `webui.py`. This is a FastAPI script generated by Anthropic's Claude, which doubles as a webserver. Any content on this website has been AI Assisted by Anthropic's Claude.
8. go to `localhost:8080` for the WebUI, and `localhost:8080/docs` for the API documentation made by FastAPI.
